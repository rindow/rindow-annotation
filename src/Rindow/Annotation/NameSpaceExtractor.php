...
class NameSpaceExtractor
{
    ...
    protected function parseUseStatement($currentNameSpace)
    {
        ...
        $text = '';
        list($code,$text) = $this->getToken();
        if($code==T_FUNCTION || $code==T_CONST) {
            $skipImport = true;
            $this->nextToken();
        }
        while(true) {
            $name = $this->parseName();
            if(!$inGroup && empty($name))
                throw new Exception\DomainException('use statement syntax error "'.$text.'" after "use" in '.$this->filename);
            $importName = trim($name,'\\');
            $alias = null;
            list($code,$text) = $this->getToken();
            if($code=='{') {
                if($inGroup || substr($name, -1,1)!='\\') {
                    throw new Exception\DomainException('use statement syntax error "'.$text.'" after "use" in '.$this->filename);
                }
                $inGroup = true;
                $this->nextToken();
                $prefixNamespace = $importName;
                $importName = $this->parseName();
                list($code,$text) = $this->getToken();
            }
            if($code==T_AS) {
                $this->nextToken();
            ...
            if(!$skipImport) {
                if(!empty($importName)) {
                    $importName = trim($prefixNamespace.'\\'.$importName,'\\');
